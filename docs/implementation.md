...
`POST /api/collection` 接收 `{"url":"<合集分享文案或链接>"}`，返回合集 ID、标题、封面、总数和 `entries`。每个条目包含视频 ID、标题、规范 URL、封面及可选时长，此阶段不获取所有视频的媒体流。

`normalize_url()` 新增 `space.bilibili.com` 主机和两种 UP 主合集路径的严格校验，旧式 `collectiondetail?sid=` 转成标准 `lists/<id>?type=season`。短链接仍校验跳转目标；输入 `type=series` 会被拒绝。

从合集成员 BV / av 链接进入时，`analyze_collection()` 先查询 `x/web-interface/view`，读取 `ugc_season.sections[].episodes`，按照原有 section 和 episode 顺序整理视频。如果目录数量与 `ep_count` 一致，则直接返回；否则根据合集 ID、用户 ID 分页请求 `x/polymer/web-space/seasons_archives_list`。直接合集链接也使用分页接口，每页请求 30 条，按 BV 号去重。总数超过 200、页数据为空导致目录不足、重复页面导致无法前进等情况都会给出错误，不静默返回残缺目录。

前端 `loadCollection()` 渲染目录和复选框；批量解析按钮依次调用既有 `/api/analyze`，每条结果保存独立解析 ID、画质和字幕选项。单个条目异常在该行显示，随后继续解析下一条。停止操作设置标记，在当前请求完成后退出循环，不取消已发出的请求。解析过程没有引入新的后台任务池，也没有创建下载任务；逐条下载仍调用 `/api/download`。

为容纳 200 条合集解析结果，解析缓存容量由 100 增加到 300，过期时间仍为 1800 秒。合集列表与已选状态只在浏览器内保存，关闭页面不会恢复批量进度。对于超过十个视频的下载需求，用户需要在已有下载完成后继续入队；本版不提供整合集自动排队。

新增六项测试覆盖新旧合集地址、成员视频恢复目录、分页合并与封面保留、不完整目录、200 条上限和无合集视频；测试总数为 24 项。
