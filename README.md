...
已通过 24 项自动测试、JavaScript 语法检查和 Chromium 无头浏览器页面检查。使用 av170001 的第 1 P 完成真实解析、下载与合并；ffprobe 验证输出包含 HEVC 视频和 AAC 音频，文件下载接口的 SHA-256 与磁盘文件一致。此结果不代表所有视频、账号和网络环境均可下载。
...
粘贴 UP 主合集分享文案或链接，点击“解析合集”。包含“合集”的分享文案以及 `space.bilibili.com` 链接，也会通过主解析按钮进入合集模式。支持新版 `/用户ID/lists/合集ID`、旧版 `/用户ID/channel/collectiondetail?sid=合集ID`，以及合集内任意 BV / av 视频链接。很多合集短链接实际跳转到其中一集，面板会从该视频元信息中恢复合集目录。

读取目录后勾选条目，点击“批量解析所选”。每集按顺序解析画质和字幕，失败不会中断后续条目；“停止后续解析”会等待当前条目返回后停止。解析完成的条目可分别选择画质、勾选字幕并加入下载队列。此功能不会自动下载整个合集；既有的两个并发下载、十个未完成任务上限仍然生效。

单个合集上限为 200 个视频，多 P 默认解析第 1 P。批量解析由当前浏览器页面驱动，关闭或刷新页面不会保存合集列表。解析记录有效 30 分钟，最多保留 300 条；过期后需重新解析。暂不支持 series 播放列表、收藏夹或番剧整季链接。

提供的 `https://b23.tv/Oyed6ZX` 已验证能恢复《【官方中字】尼古喵喵 （全12集）》的完整目录；浏览器实测 12 集全部解析成功，手机布局无横向溢出。
