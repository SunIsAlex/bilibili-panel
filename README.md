...
Python + yt-dlp + FFmpeg，无前端构建步骤。支持 BV / av / ep 号、b23.tv 短链接、番剧与影视单集、指定分 P、可用画质选择、双任务并发队列、进度、任务封面和文件保存。

## 技术文档

[项目实现详解](docs/implementation.md)：架构、接口、链接解析、音视频合并、Cookie 授权、任务队列与缩略图实现。
...
已通过 18 项自动测试、JavaScript 语法检查和 Chromium 无头浏览器页面检查。使用 av170001 的第 1 P 完成真实解析、下载与合并；ffprobe 验证输出包含 HEVC 视频和 AAC 音频，文件下载接口的 SHA-256 与磁盘文件一致。此结果不代表所有视频、账号和网络环境均可下载。
...
支持粘贴包含单个链接的 Bilibili 分享文案。单集解析失败时会尝试查询播放信息诊断 DRM；确认受保护时显示明确提示，不尝试获取密钥或解密。`ep810659` 已验证为此类内容，登录会员也不代表能通过本面板下载。

## 字幕下载

解析视频后，可在“字幕”区域选择一种或多种语言：点击“加入下载队列”同时下载视频和字幕，或点击“仅下载字幕”跳过视频媒体下载。完成后，每个字幕文件都有独立保存链接，优先保存为 SRT，兼容解析器提供的 VTT / ASS / SSA。

字幕来源是平台返回的外挂字幕，不包含弹幕，也不对画面内嵌文字进行 OCR；没有字幕时显示明确提示。部分字幕需要有效登录 Cookie。字幕以独立文件保存，不烧录或封装进视频。仅字幕模式无需 FFmpeg，但仍须完成视频解析，不能用于绕过 DRM 或播放权限限制。

已使用 `BV12N4y1M7rh` 实测下载简体中文 SRT；任务元数据与视频任务一样保存在内存中。
